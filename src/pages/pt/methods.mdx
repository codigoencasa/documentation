...
```mermaid
fluxograma LR
subestado do subgráfico1[usuário de Conversação A]
...
%% Link *to* flowStateA: a direção do flowStateA é mantida
estado --> flowStateA
%% Link * within * flowStateB:
%% flowStateB herda a direção do Gráfico de nível superior (LR)
estado - - - > topState2
fim
...
```mermaid
fluxograma LR
subestado do subgráfico2[usuário de Conversação B]
style subState2 fill:##fdf8e1, stroke:##ffffef, stroke-width:2px
subgrafo flowState2A
direção TB
topState21[state] --> bottom1[addAnswer]
fim
subgrafo flowState2B
direção TB
topState22[state] --> bottom2[addAnswer]
fim
%% ^ Esses subgrafos são idênticos, exceto pelos links para eles:

%% Link *to* flowState2A: a direção do flowState2A é mantida
estado --> flowState2A
%% Link * dentro* flowState2B:
%% flowState2B herda a direção do Gráfico de nível superior (LR)
estado - - - > topestado22
fim
```
...
```mermaid
fluxograma LR
subgrafo sub0[GlobalState]
subgrafo sub1[usuário de Conversação A]
...
%% Link *to* flowA: a direção do flowA é mantida
stateA -.- >top1
%% Link * dentro * flowB:
%% flowB herda a direção do Gráfico de nível superior (LR)
stateA -.- >top2
fim

subgrafo sub2[usuário de Conversação B]
style sub2 fill:#e2eafc, stroke:#ccdbfd, stroke-width:2px
subgrafo flowA1
direção TB
top3[estadob] -.- >bottom3[addAnswer]
fim
subgrafo fluxob1
direção TB
top4[estadob] -.- >bottom4[addAnswer]
fim
%% ^ Esses subgrafos são idênticos, exceto pelos links para eles:

%% Link *to * flowA1: a direção do flowA1 é mantida
estadob -.- >top3
%% Link * dentro * fluxob1:
%% fluxob1 herda a direção do Gráfico de nível superior (LR)
estadob -.- >top4
fim
...
```mermaid
fluxograma TB
flowWelcome[fluxo-bem-vindo.ts] -->/Inicia flow-a/flowA[flow-A. ts]
